...
function styleRows(cur: StyleBaseline, base: StyleBaseline | null): string {
  const rows: [string, (b: StyleBaseline) => number, string, number][] = [
    ...
    ['Direksiyon düzeltme (SRR)', (b) => b.srr, '/dk', 1],
...
export function renderReport(host: HTMLElement, d: ReportData, map: MapRenderer, actions: ReportActions) {
  ...
  const head = el('div', {
    ...
    html: `<div class="rp-ring">${scoreRing(r.overall, 150, `Not ${r.grade}`)}</div>
      <div class="rp-title"><h1>${esc(title)}</h1>
        ...
        ${verdict}
        <div class="rp-badges"><span class="risk risk-${r.risk}">Risk: ${r.risk}</span>${r.styleAdherence != null ? `<span class="chip">Stil uyumu ${r.styleAdherence}</span>` : ''}${r.confidence < 0.8 ? `<span class="chip muted">Kısa oturum güveni %${Math.round(r.confidence * 100)}</span>` : ''}</div>
        ${karneLine}${badges}
  ...
  const summary = el('div', {
    ...
    html: `<div class="grid2">
      <div class="card"><h3>5 eksen</h3>${compBars}<p class="muted small">Ağırlıklar: Güvenlik %30 · Kural %25 · Tarama %20 · Pürüzsüzlük %15 · Görev %10</p></div>
  ...
  detail.append(
    el('div', {
      class: 'card',
      html: `<h3>Yöntem</h3><p class="muted small">Olaylar sürüş sırasında kural tabanlı dedektörlerle çıkarılır (şerit/dönüş için Ayna→Sinyal→Manevra sırası, ışık fazı, DUR levhasında en düşük hız, TTC ve takip mesafesi, yanal/boylamsal ivme). Alt metrikler 0–100 aralığına ölçeklenir, bileşen içinde ağırlıklandırılır ve Güvenlik %30 · Kural %25 · Tarama %20 · Pürüzsüzlük %15 · Görev %10 ağırlıklarıyla birleştirilir. Kısa oturumlar düşük güvenle 70'e doğru çekilir; ağır ihlaller toplam puana tavan uygular. Tarama ölçümü ${esc('bakış tuşları / webcam kafa takibi / tek tuş')} proxy'sidir — klinik göz takibi değildir.</p>`,
    })
  );
  pages.push(detail);
  ...
  const stylePage = el('div', {
    class: 'page',
    html: `<div class="card"><h3>Kişisel sürüş profili karşılaştırması</h3>${d.baseline ? '' : '<p class="muted">Henüz kalibre edilmiş bir profil yok. Kalibrasyon Programı\'nı tamamlayınca bu oturum kişisel ortalamanla karşılaştırılır.</p>'}${styleRows(r.style, d.baseline)}<p class="muted small">Stil uyumu güvenlik puanını şişirmez; yalnızca alışkanlıklarındaki değişimi gösterir.</p></div>`,
  });
